...
- **Pós-venda**: todo job que fecha entra automaticamente como pendência de pós-venda (sem travar nada — os jobs continuam do jeito que sempre foram). Na aba Dashboard, a seção **Pós-venda** lista essas pendências (cliente, valor fechado, há quantos dias fechou) com dois botões — "✓ Feito" ou "✕ Não vai" — pra registrar se rolou o acompanhamento pós-fechamento ou não. Fica com métricas próprias, separadas do resto (quantidade pendente, feito, não vai fazer, e % de pós-venda realizado), sem misturar com os KPIs comerciais de cima.
- **Fechamento direto (sem proposta customizada)**: um checkbox no job pra marcar quando o cliente fechou uma mídia direto, sem passar por uma proposta customizada da marketing. O job continua normal no quadro (avança fase, fecha, aparece nos filtros), mas o **Dashboard conta ele à parte**: fica de fora do valor orçado/fechado, % de conversão, meta por praça, segmentação, funil, comparativo anual e das quebras de conversão por analista/executivo — só entra nos números operacionais do topo (total de jobs, em andamento, atrasados) e numa seção própria **"Fechamentos diretos"** (quantidade, valor fechado, ticket médio). Assim esse tipo de fechamento não infla os indicadores que medem o trabalho de proposta customizada da marketing.
