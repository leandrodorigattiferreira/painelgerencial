...
Cada job também tem praça (CAM/RIB/SCA/VAR/CE), origem (Solicitação interna/EP Labs/Proativo), analista, executivo de conta, prioridade, situação, marcação de **conta especial (estratégica)**, três datas (**solicitação**, **prazo**, **entrega**), **briefing recebido**, um **checklist** (manual ou gerado por IA a partir do briefing), próximos passos, um link de **Plano de Mídia** e uma lista de **planilhas** (nome + link — não há upload de arquivo dentro do artifact, então entram por link). **% de conclusão** e **alerta de prazo** são calculados automaticamente a partir da fase do job.
